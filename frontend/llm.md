...
## Boundary rules (TigerBeetle-ish safety model)
Core uses two categories of failure:

### Expected invalid input (soft failure)
These can happen due to user gestures/races (e.g., “split at index 0”, “stack already removed”).
They should NOT crash the system; return null / [] / do nothing:
- missing stack IDs passed from UI
- empty stack “pop”
- split index out of range
- takeRange out of range

### Invariant violations (hard failure)
These indicate programmer error or corruption. Core should assert + throw:
- non-integer indices
- negative sizes / impossible postconditions
- Engine bookkeeping mismatch (stackIds vs stacks map)
- “should never happen” states

Core implements this via core/assert.ts and uses paired assertions:
- assert at the boundary (inputs / preconditions)
- assert at the exit (postconditions)
