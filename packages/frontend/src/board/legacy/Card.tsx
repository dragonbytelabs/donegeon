...
export function LegacyCard(props: LegacyCardProps) {
  ...
  return (
    <div
      class={[
        "relative w-[120px] select-none overflow-hidden rounded-xl border bg-white/90 text-slate-900 shadow-lg",
        accent,
        props.active ? "ring-2 ring-emerald-400" : "",
        props.collapsed ? "h-[38px]" : "h-[160px]"
      ].join(" ")}
    >
      {/* handle + unstack (task-only) */}
      <Show when={props.showHandle && !props.collapsed}>
        <div class="absolute left-[-18px] top-[42px] flex flex-col items-center gap-2">
          <div class="h-[74px] w-[16px] rounded-l-lg bg-slate-200/50 shadow-inner" />
          <button
      ...
      <Show when={!props.collapsed}>
        <div class="flex flex-col items-center justify-center gap-2 px-3 pt-1">
          <div class="text-4xl leading-none">{props.emoji}</div>
          <div class="text-[13px] font-black tracking-tight">{props.name}</div>
